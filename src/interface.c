...
create_etage_panne (void)
{
  ...
  gtk_window_set_title (GTK_WINDOW (etage_panne), _("Etage contenant des pannes"));

  fixed10 = gtk_fixed_new ();
  gtk_widget_show (fixed10);
  gtk_container_add (GTK_CONTAINER (etage_panne), fixed10);

  treeview6 = gtk_tree_view_new ();
  gtk_widget_show (treeview6);
  gtk_fixed_put (GTK_FIXED (fixed10), treeview6, 0, 0);
  gtk_widget_set_size_request (treeview6, 520, 320);

  /* Store pointers to all widgets, for use by lookup_widget(). */
  GLADE_HOOKUP_OBJECT_NO_REF (etage_panne, etage_panne, "etage_panne");
  GLADE_HOOKUP_OBJECT (etage_panne, fixed10, "fixed10");
  GLADE_HOOKUP_OBJECT (etage_panne, treeview6, "treeview6");
